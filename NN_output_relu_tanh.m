function [y] =  NN_output_relu_tanh(x,offset,scale_factor,name)
...
weight_matrix = zeros(network_structure(no_of_hidden_layers+1), network_structure(no_of_hidden_layers));
bias_matrix = zeros(network_structure(no_of_hidden_layers+1),1);

% READING THE WEIGHT MATRIX
for j = 1:network_structure(no_of_hidden_layers+1)
    for k = 1:network_structure(no_of_hidden_layers)
        weight_matrix(j,k) = file_data(pointer);
        pointer = pointer + 1;
    end
    bias_matrix(j) = file_data(pointer);
